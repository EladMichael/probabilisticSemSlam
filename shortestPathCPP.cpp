...
MurtyHyp *boundingUpdateCPP(const MurtyHyp *parentHyp, ScratchSpace &workMem,const size_t curUnassignedCol, size_t numRow2Scan, const size_t numVarCol, const size_t numDim) {
/*SHORTESTPATHUPDATECPP
 *
 * This is a realization of the bounding (i.e. non-solve) inheriting dual variables as
 * described in * M. L. Miller, H. S. Stone, and I. J. Cox, "Optimizing Murty's ranked
 * assignment method," IEEE Transactions on Aerospace and Electronic
 * Systems, vol. 33, no. 3, pp. 851-862, Jul. 1997.
 * to efficiently implement Murty's algorithm. This function is only used
 * in the k-best 2D assignment algorithm, not in the regular 2D assignment
 * algorithm. The return value is the new, bounded but unsolved, solution. The dual variables from
 * the parent hypothesis parentHyp are inherited and modified. This
 * function is called by the split function. If the problem is infeasible or the new gain is beyond the cutoff,
 * then the gain in problemSol is set to -1.
 *
 */
    ...
    size_t row2Search,col2Search;
    row2Search = problemSol->row4col[curUnassignedCol];
    col2Search = problemSol->col4row[row2Search]; 

    //Remove the association of the current row/column.
    problemSol->col4row[row2Search]=-1;
    problemSol->row4col[col2Search]=-1;

    // Lower bound the assignment of the new hyp, with curUnassignedRow the only unassigned row.
    // Use the sensitivities (duals).
    double reducedCost;

    /*Scan all of the rows which are not forbidden in this column*/
    double minRowVal = numeric_limits<double>::infinity();
    for(size_t curRowScan=0; curRowScan<numRow2Scan; curRowScan++) {
        curRow=static_cast<size_t>(workMem.Row2Scan[curRowScan]);
            
        if(workMem.forbiddenActiveRows[curRow] == false) {
            reducedCost = workMem.C[curRow+col2Search*numDim]-problemSol->u[col2Search]-problemSol->v[curRow];
            if(reducedCost < minRowVal){
                minRowVal = reducedCost;
            }
        }
    }

    /*Scan all columns after this one*/
    double minColVal = numeric_limits<double>::infinity();
    for(size_t curCol=col2Search+1; curCol < numDim; curCol++) {
            
        reducedCost = workMem.C[row2Search+curCol*numDim]-problemSol->u[curCol]-problemSol->v[row2Search];
        if(reducedCost < minColVal){
            minColVal = reducedCost;
        }
    }
    
    double maxVal = std::max(minRowVal,minColVal);

    if(maxVal==numeric_limits<double>::infinity()) {
       /* If the minimum cost column is not finite, then the
        * problem is not feasible.*/
    ...
    problemSol->gain = parentHyp-> gain + maxVal;
    problemSol->solved = false;
 ...
 */    
    ...
    if((newHyp->gain==-1) || workMem.cutHyp(newHyp->gain)) {
        delete newHyp;
    }
    else {
    ...
    for(curCol=activeCol+1;curCol<numVarCol;curCol++){
        ...
        /*If it is not a missed detection, add it to the queue*/
        if((newHyp->gain==-1) || workMem.cutHyp(newHyp->gain)) {delete newHyp;}
...
size_t kBest2DCutoff(const size_t k,const size_t numRow,const size_t numCol,
    const bool maximize,const double *C, ScratchSpace &workMem,
    ptrdiff_t *col4rowBest,ptrdiff_t *row4colBest,double *gainBest,double cutoff) {

    workMem.toCut = true;
    workMem.maximize = maximize;
    ...
    /* Adjust for shifting that was done to make everything positive.*/
    if(maximize==false) {
        workMem.cutoffGain = gainBest[0]+cutoff;
        gainBest[0]=gainBest[0]+CDelta;
    } else {
        workMem.cutoffGain = gainBest[0]-cutoff;
        gainBest[0]=-gainBest[0]+CDelta;
    ...
    for(curSweep=1;curSweep<k;curSweep++) {
        ...
        /* Adjust for shifting that was done to make everything positive.*/
            if(maximize==false) {
                gainBest[curSweep]=gainBest[curSweep]+CDelta;
                if(gainBest[curSweep] > gainBest[0]+cutoff){
                    break;
                }
            } else {
                gainBest[curSweep]=-gainBest[curSweep]+CDelta;
